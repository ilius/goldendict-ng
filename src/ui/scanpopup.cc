...
ScanPopup::ScanPopup(
	...
	connect(
		definition,
		&ArticleView::inspectSignal,
		this,
		&ScanPopup::inspectElementWhenPinned,
	);
	connect(
		definition,
	...
	connect(
		this, &ScanPopup::closeMenu, definition, &ArticleView::closePopupMenu,
	);
	connect(
		definition,
	...
	connect(
		definition, &ArticleView::typingEvent, this, &ScanPopup::typingEvent,
	);
	...
	connect(
		...
		this,
		&ScanPopup::translateInputChanged,
	);
	...
	connect(
		...
		this,
		&ScanPopup::translateInputFinished,
	);

	connect(
		ui.translateBox->wordList(),
		&QListWidget::itemClicked,
		this,
		&ScanPopup::wordListItemActivated,
	);
	...
	connect(
		...
		this,
		&ScanPopup::showStatusBarMessage,
	);
	...
	if (cfg.lastPopupGroupId == Instances::Group::AllGroupId) {
		...
		dictionaryBar.setMutedDictionaries(
			grp ? &grp->popupMutedDictionaries : 0,
		);
	}
	...
	connect(
		&dictionaryBar,
		SIGNAL(editGroupRequested()),
		this,
		SLOT(editGroupRequested()),
	);
	connect(
		this,
		&ScanPopup::closeMenu,
		&dictionaryBar,
		&DictionaryBar::closePopupMenu,
	);
	connect(
		&dictionaryBar,
  ...
  setAttribute( Qt::WA_NoSystemBackground );
	...
	connect(
		&escapeAction, &QAction::triggered, this, &ScanPopup::escapePressed,
	);
	...
	connect(
		&focusTranslateLineAction,
		&QAction::triggered,
		this,
		&ScanPopup::focusTranslateLine,
	);
	...
	switchExpandModeAction.setShortcuts(
		QList<QKeySequence>() << QKeySequence(Qt::CTRL | Qt::Key_8)
	...
	connect(
		&switchExpandModeAction,
		&QAction::triggered,
		this,
		&ScanPopup::switchExpandOptionalPartsMode,
	);
	...
	connect(
		definition, &ArticleView::titleChanged, this, &ScanPopup::titleChanged,
	);
	...
	connect(
		&mouseGrabPollTimer, &QTimer::timeout, this, &ScanPopup::mouseGrabPoll,
	);
#ifdef Q_OS_MAC
	...
	connect(scanFlag, &ScanFlag::requestScanPopup, this, [=] {
...
void ScanPopup::refresh() {
	...
	ui.groupList->setCurrentGroup(OldGroupID);
	// This does nothing if OldGroupID doesn't exist;
...
[[deprecated]] void
ScanPopup::handleInputWord(QString const & str, bool forcePopup) {
	Config::InputPhrase sanitizedPhrase =
		cfg.preferences.sanitizeInputPhrase(str);
...
void ScanPopup::engagePopup(bool forcePopup, bool giveFocus) {
	...
	definition->setSelectionBySingleClick(
	...
	if (!isVisible()) {
		...
		if (!ui.pinButton->isChecked()) {
			...
			if (currentPos.x() + 4 + windowSize.width()
				<= desktop.topRight().x()) {
				x = currentPos.x() + 4;
			...
			if (currentPos.y() + 15 + windowSize.height()
				<= desktop.bottomLeft().y()) {
				/// Try the to-the-bottom placement
				y = currentPos.y() + 15;
			} else if (currentPos.y() - 15 - windowSize.height() >= desktop.y()) {
	...
	/// wants
	ui.translateBox->setText(
		Folding::escapeWildcardSymbols(inputPhrase.phrase), false,
	);
...
void ScanPopup::translateInputFinished() {
	inputPhrase.phrase = Folding::unescapeWildcardSymbols(
		ui.translateBox->translateLine()->text().trimmed(),
	);
...
vector<sptr<Dictionary::Class>> const & ScanPopup::getActiveDicts() {
	...
	for (const auto & activeDict : activeDicts) {
		if (!mutedDictionaries->contains(
				QString::fromStdString(activeDict->getId())
			)) {
...
bool ScanPopup::eventFilter(QObject * watched, QEvent * event) {
	...
	if (event->type() == QEvent::KeyPress
		&& watched != ui.translateBox->translateLine()) {
...
void ScanPopup::reactOnMouseMove(QPoint const & p) {
	if (geometry().contains(p)) {
		...
		if (/*watched == this &&*/
			!frameGeometry()
				 .adjusted(-proximity, -proximity, proximity, proximity)
				 .contains(p)) {
...
void ScanPopup::showStatusBarMessage(
...
void ScanPopup::on_sendWordToFavoritesButton_clicked() {
	...
	unsigned groupId = ui.groupList->getCurrentGroup();
	auto current_exist =
		isWordPresentedInFavorites(definition->getTitle(), groupId);
	//if current_exist=false( not exist ),  after click ,the word should be in the favorite which is blueStar
	ui.sendWordToFavoritesButton->setIcon(
		!current_exist ? blueStarIcon : starIcon,
	);
	emit sendWordToFavorites(
		definition->getTitle(), cfg.lastPopupGroupId, current_exist,
	);
}
...
void ScanPopup::titleChanged(ArticleView *, QString const & title) {
	...
	ui.sendWordToFavoritesButton->setIcon(
		isWordPresentedInFavorites(title, groupId) ? blueStarIcon : starIcon,
	);
}

bool ScanPopup::isWordPresentedInFavorites(
	QString const & word, unsigned groupId
) const {
	QString folder = GlobalBroadcaster::instance()->groupFolderMap[groupId];

	return GlobalBroadcaster::instance()->folderFavoritesMap[folder].contains(
		word,
	);
}
