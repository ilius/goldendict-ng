...
void MainWindow::changeWebEngineViewFont() {
	if (cfg.preferences.webFontFamily.isEmpty()) {
		QWebEngineProfile::defaultProfile()->settings()->resetFontFamily(
			QWebEngineSettings::StandardFont
		);
	} else {
		QWebEngineProfile::defaultProfile()->settings()->setFontFamily(
			QWebEngineSettings::StandardFont, cfg.preferences.webFontFamily
		);
	}
}
...
MainWindow::MainWindow(Config::Class & cfg_):
	...
	history(
		History::Load(),
		cfg_.preferences.maxStringsInHistory,
		cfg_.maxHeadwordSize
	),
	dictionaryBar(
		this,
		configEvents,
		cfg.editDictionaryCommandLine,
		cfg.preferences.maxDictionaryRefsInContextMenu
	),
	...
	QWebEngineProfile::defaultProfile()->installUrlSchemeHandler(
		"gdlookup", localSchemeHandler
	);
	QWebEngineProfile::defaultProfile()->installUrlSchemeHandler(
		"bword", localSchemeHandler
	);
	QWebEngineProfile::defaultProfile()->installUrlSchemeHandler(
		"entry", localSchemeHandler
	);

	iframeSchemeHandler = new IframeSchemeHandler(this);
	QWebEngineProfile::defaultProfile()->installUrlSchemeHandler(
		"ifr", iframeSchemeHandler
	);
	...
	for (int i = 0; i < localSchemes.size(); i++) {
		QWebEngineProfile::defaultProfile()->installUrlSchemeHandler(
			localSchemes.at(i).toLatin1(), resourceSchemeHandler
		);
	}

	QWebEngineProfile::defaultProfile()->setUrlRequestInterceptor(
		new WebUrlRequestInterceptor(this)
	);
	...
	navForward =
		navToolbar->addAction(QIcon(":/icons/next.svg"), tr("Forward"));
	...
	groupListInToolbar->setSizePolicy(
	...
	enableScanningAction = navToolbar->addAction(
		QIcon(":/icons/wizard.svg"), tr("Enable Scanning")
	);
	enableScanningAction->setCheckable(true);

	navToolbar->widgetForAction(enableScanningAction)
		->setObjectName("scanPopupButton");

	navToolbar->addSeparator();

	// sound
	navPronounce = navToolbar->addAction(
		QIcon(":/icons/playsound_full.png"), tr("Pronounce Word (Alt+S)")
	);
	...
	navToolbar->widgetForAction(navToolbar->addSeparator())
		->setObjectName("separatorBeforeZoom");

	zoomIn = navToolbar->addAction(
		QIcon(":/icons/icon32_zoomin.png"), tr("Zoom In")
	);
	zoomIn->setShortcuts(
		QList<QKeySequence>() << QKeySequence::ZoomIn << QKeySequence("Ctrl+=")
	);
	navToolbar->widgetForAction(zoomIn)->setObjectName("zoomInButton");

	zoomOut = navToolbar->addAction(
		QIcon(":/icons/icon32_zoomout.png"), tr("Zoom Out")
	);
	zoomOut->setShortcut(QKeySequence::ZoomOut);
	navToolbar->widgetForAction(zoomOut)->setObjectName("zoomOutButton");

	zoomBase = navToolbar->addAction(
		QIcon(":/icons/icon32_zoombase.png"), tr("Normal Size")
	);
	...
	// named separator (to be able to hide it via CSS)
	navToolbar->widgetForAction(navToolbar->addSeparator())
		->setObjectName("separatorBeforeSave");

	navToolbar->addAction(ui.saveArticle);
	navToolbar->widgetForAction(ui.saveArticle)
		->setObjectName("saveArticleButton");

	navToolbar->addAction(ui.print);
	navToolbar->widgetForAction(ui.print)->setObjectName("printButton");

	navToolbar->widgetForAction(navToolbar->addSeparator())
		->setObjectName("separatorBeforeAddToFavorites");

	addToFavorites =
		navToolbar->addAction(starIcon, tr("Add current tab to Favorites"));
	navToolbar->widgetForAction(addToFavorites)
		->setObjectName("addToFavoritesButton");

	connect(
		addToFavorites,
		&QAction::triggered,
		this,
		&MainWindow::handleAddToFavoritesButton
	);
	connect(
		ui.actionAddToFavorites,
		&QAction::triggered,
		this,
		&MainWindow::addCurrentTabToFavorites
	);

	beforeOptionsSeparator = navToolbar->addSeparator();
	navToolbar->widgetForAction(beforeOptionsSeparator)
		->setObjectName("beforeOptionsSeparator");
	...
	connect(
		ui.searchPane->toggleViewAction(),
		&QAction::triggered,
		this,
		&MainWindow::updateSearchPaneAndBar
	);
	...
	connect(
		...
		this,
		&MainWindow::dictsPaneVisibilityChanged
	);
	...
	connect(
		...
		this,
		&MainWindow::foundDictsPaneClicked
	);
	...
	connect(
		...
		this,
		&MainWindow::foundDictsContextMenuRequested
	);
	...
	wordsZoomIn = ui.menuZoom->addAction(
		QIcon(":/icons/icon32_zoomin.png"), tr("Words Zoom In")
	);
	wordsZoomIn->setShortcuts(
		QList<QKeySequence>() << QKeySequence("Alt++") << QKeySequence("Alt+=")
	);
	wordsZoomOut = ui.menuZoom->addAction(
		QIcon(":/icons/icon32_zoomout.png"), tr("Words Zoom Out")
	);
	wordsZoomOut->setShortcut(QKeySequence("Alt+-"));
	wordsZoomBase = ui.menuZoom->addAction(
		QIcon(":/icons/icon32_zoombase.png"), tr("Words Normal Size")
	);
	wordsZoomBase->setShortcut(QKeySequence("Alt+0"));

	connect(wordsZoomIn, &QAction::triggered, this, &MainWindow::doWordsZoomIn);
	connect(
		wordsZoomOut, &QAction::triggered, this, &MainWindow::doWordsZoomOut
	);
	connect(
		wordsZoomBase, &QAction::triggered, this, &MainWindow::doWordsZoomBase
	);

	// tray icon
	connect(
		trayIconMenu.addAction(tr("Show &Main Window")),
		&QAction::triggered,
		this,
	...
	focusTranslateLineAction.setShortcuts(
		QList<QKeySequence>() << QKeySequence("Alt+D") << QKeySequence("Ctrl+L")
	);
	...
	connect(
		tabListMenu,
		&MRUQMenu::requestTabChange,
		ui.tabWidget,
		&MainTabWidget::setCurrentIndex
	);
	...
	connect(
		&closeAllTabAction, &QAction::triggered, this, &MainWindow::closeAllTabs
	);
	...
	connect(
		&switchToPrevTabAction,
		&QAction::triggered,
		this,
		&MainWindow::switchToPrevTab
	);
	...
	connect(
		...
		&QAction::triggered,
		this,
		&MainWindow::addAllTabsToFavorites
	);
	...
	connect(
		&showDictBarNamesAction,
		&QAction::triggered,
		this,
		&MainWindow::showDictBarNamesTriggered
	);
	...
	connect(
		...
		&QAction::triggered,
		this,
		&MainWindow::useSmallIconsInToolbarsTriggered
	);
	...
	connect(
		...
		&QAction::triggered,
		this,
		&MainWindow::toggleMenuBarTriggered
	);
	...
	Instances::Group const * igrp =
		groupInstances.findGroup(cfg.lastMainGroupId);
	...
	connect(
		this,
		&MainWindow::clickOnDictPane,
		&dictionaryBar,
		&DictionaryBar::dictsPaneClicked
	);
	...
	connect(
		...
		&QAction::triggered,
		this,
		&MainWindow::dictionaryBarToggled
	);
	...
	connect(
		&dictionaryBar,
		&DictionaryBar::editGroupRequested,
		this,
		&MainWindow::editCurrentGroup
	);

	connect(
		&dictionaryBar,
		&DictionaryBar::showDictionaryInfo,
		this,
		&MainWindow::showDictionaryInfo
	);

	connect(
		&dictionaryBar,
		&DictionaryBar::showDictionaryHeadwords,
		this,
		&MainWindow::showDictionaryHeadwords
	);

	connect(
		&dictionaryBar,
		&DictionaryBar::openDictionaryFolder,
		this,
		&MainWindow::openDictionaryFolder
	);

	// Favorites

	ui.favoritesPaneWidget->setUp(&cfg, ui.menuFavorites);
	ui.favoritesPaneWidget->setSaveInterval(
		cfg.preferences.favoritesStoreInterval
	);
	...
	connect(
		...
		&QDockWidget::visibilityChanged,
		this,
		&MainWindow::updateFavoritesMenu
	);
	connect(
		ui.showHideFavorites,
		&QAction::triggered,
		this,
		&MainWindow::toggle_favoritesPane
	);
	...
	connect(
		ui.historyPaneWidget,
		&HistoryPaneWidget::historyItemRequested,
		this,
		&MainWindow::showHistoryItem
	);
	...
	connect(
		...
		&QDockWidget::visibilityChanged,
		this,
		&MainWindow::updateHistoryMenu
	);
	connect(
		ui.showHideHistory,
		&QAction::triggered,
		this,
		&MainWindow::toggle_historyPane
	);
	...
	if (cfg.preferences.enableTrayIcon) {
		trayIcon = new QSystemTrayIcon(
			QIcon::fromTheme(
				"goldendict-tray", QIcon(":/icons/programicon_old.png")
			),
			this
		);
	...
	connect(
		...
		&QAction::triggered,
		this,
		&MainWindow::editDictionaries
	);

	connect(
		ui.preferences, &QAction::triggered, this, &MainWindow::editPreferences
	);

	connect(
		ui.visitHomepage, &QAction::triggered, this, &MainWindow::visitHomepage
	);
	...
	connect(
		groupListInDock,
		&GroupComboBox::currentIndexChanged,
		this,
		&MainWindow::currentGroupChanged
	);
	...
	connect(
		...
		this,
		&MainWindow::translateInputChanged
	);
	...
	connect(
		...
		this,
		&MainWindow::wordListSelectionChanged
	);

	connect(
		translateBox->wordList(),
	...
	connect(
		...
		&QListWidget::itemClicked,
		this,
		&MainWindow::wordListItemActivated
	);

	connect(
		ui.wordList,
		&WordList::statusBarMessage,
		this,
	...
	connect(
		...
		&QListWidget::itemSelectionChanged,
		this,
		&MainWindow::dictsListSelectionChanged
	);
	...
	connect(
		...
		this,
		&MainWindow::dictsListItemActivated
	);

	connect(
		&configEvents,
		&Config::Events::mutedDictionariesChanged,
		this,
	...
	connect(
		&ftsIndexing,
		&FTS::FtsIndexing::newIndexingName,
		this,
	...
	connect(
		...
		this,
		[this](Config::InputPhrase const & phrase) {
			phraseReceived(phrase, WildcardsAreAlreadyEscaped);
		}
	);

	connect(
		scanPopup, &ScanPopup::inspectSignal, this, &MainWindow::inspectElement
	);
	connect(
		scanPopup,
		&ScanPopup::forceAddWordToHistory,
		this,
		&MainWindow::forceAddWordToHistory
	);
	connect(
		scanPopup,
		&ScanPopup::showDictionaryInfo,
		this,
		&MainWindow::showDictionaryInfo
	);
	connect(
		scanPopup,
		&ScanPopup::openDictionaryFolder,
		this,
		&MainWindow::openDictionaryFolder
	);
	connect(
		scanPopup,
		&ScanPopup::sendWordToHistory,
		this,
		&MainWindow::addWordToHistory
	);
	connect(
		this,
		&MainWindow::setPopupGroupByName,
		scanPopup,
		&ScanPopup::setGroupByName
	);
	connect(
		scanPopup,
		&ScanPopup::sendWordToFavorites,
		this,
		&MainWindow::addWordToFavorites
	);

#ifdef Q_OS_MAC
	macClipboard = new gd_clipboard(this);
	connect(
		macClipboard, &gd_clipboard::changed, this, &MainWindow::clipboardChange
	);
#endif
	...
	connect(enableScanningAction, &QAction::toggled, this, [=](bool on) {
		...
		if (!MacMouseOver::isAXAPIEnabled())
			mainStatusBar->showMessage(
				tr("Accessibility API is not enabled"),
				10000,
	...
	connect(
		&newReleaseCheckTimer,
		&QTimer::timeout,
		this,
		&MainWindow::checkForNewRelease
	);
...
void MainWindow::clipboardChange(QClipboard::Mode m) {
	...
	if (m == QClipboard::Selection) {
		...
		if (cfg.preferences.enableScanPopupModifiers
			&& !KeyboardState::checkModifiersPressed(
				cfg.preferences.scanPopupModifiers
			)) {
			return;
		}
...
void MainWindow::updateSearchPaneAndBar(bool searchInDock) {
	...
	setTranslateBoxTextAndKeepSuffix(
		text, WildcardsAreAlreadyEscaped, DisablePopup
	);
	focusTranslateLine();
}
...
void MainWindow::mousePressEvent(QMouseEvent * event) {
	...
	QString str =
		QApplication::clipboard()->text(subtype, QClipboard::Selection);
...
void MainWindow::addGroupComboBoxActionsToDialog(
	QDialog * dialog, GroupComboBox * pGroupComboBox
) {
	dialog->addActions(pGroupComboBox->getExternActions());
}

void MainWindow::removeGroupComboBoxActionsFromDialog(
	QDialog * dialog, GroupComboBox * pGroupComboBox
) {
	QList<QAction *> actions = pGroupComboBox->getExternActions();
	for (QList<QAction *>::iterator it = actions.begin(); it != actions.end();
		 ++it)
...
void MainWindow::applyQtStyleSheet(
	...
	if (!addonStyle.isEmpty()) {
		QString name = Config::getStylesDir() + addonStyle + QDir::separator()
			+ "qt-style.css";
		QFile addonCss(name);
...
void MainWindow::updateTrayIcon() {
	...
	if (trayIcon) {
		...
		trayIcon->setIcon(
			enableScanningAction->isChecked() ?
				QIcon::fromTheme(
					"goldendict-scan-tray",
					QIcon(":/icons/programicon_scan.png")
				) :
				QIcon::fromTheme(
					"goldendict-tray", QIcon(":/icons/programicon_old.png")
				)
...
void MainWindow::applyProxySettings() {
	if (cfg.preferences.proxyServer.enabled
		&& cfg.preferences.proxyServer.useSystemProxy) {
		...
		if (!cfg.preferences.proxyServer.systemProxyUser.isEmpty()) {
			...
			proxies.first().setPassword(
				cfg.preferences.proxyServer.systemProxyPassword
			);
		}
...
void MainWindow::setupNetworkCache(int maxSize) {
	...
	qint64 const maxCacheSizeInBytes =
		maxSize <= 0 ? qint64(0) : static_cast<qint64>(maxSize) << 20;

	if (QAbstractNetworkCache * abstractCache = articleNetMgr.cache()) {
		QNetworkDiskCache * const diskCache =
			qobject_cast<QNetworkDiskCache *>(abstractCache);
	...
	if (!QDir().mkpath(cacheDirectory)) {
		...
		gdWarning(
			"Cannot create a cache directory %s. use default cache path.",
			cacheDirectory.toUtf8().constData()
		);
	}
...
void MainWindow::makeDictionaries() {
	...
	for (unsigned x = 0; x < dictionaries.size(); x++) {
		...
		dictionaries[x]->setSynonymSearchEnabled(
			cfg.preferences.synonymSearchEnabled
		);
	}
...
void MainWindow::updateStatusLine() {
	...
	mainStatusBar->showMessage(
		tr("%1 dictionaries, %2 articles, %3 words")
...
void MainWindow::updateGroupList() {
	...
	// Add dictionaryOrder first, as the 'All' group.
	{
		...
		Instances::complementDictionaryOrder(
			g,
			Instances::Group(
				cfg.inactiveDictionaries, dictionaries, Config::Group()
			),
			dictionaries
		);
	...
	for (int x = 0; x < cfg.groups.size(); ++x) {
		groupInstances.push_back(Instances::Group(
			cfg.groups[x], dictionaries, cfg.inactiveDictionaries
		));
		GlobalBroadcaster::instance()->groupFolderMap.insert(
			cfg.groups[x].id, cfg.groups[x].favoritesFolder
		);
	}
...
void MainWindow::updateDictionaryBar() {
	...
	if (grp) { // Should always be !0, but check as a safeguard
		if (currentId == Instances::Group::AllGroupId)
			...
			dictionaryBar.setMutedDictionaries(
				grp ? &grp->mutedDictionaries : 0
			);
		}
...
vector<sptr<Dictionary::Class>> const & MainWindow::getActiveDicts() {
	...
	Config::MutedDictionaries const * mutedDictionaries =
		dictionaryBar.getMutedDictionaries();
	if (!dictionaryBar.toggleViewAction()->isChecked()
		|| mutedDictionaries == 0)
...
void MainWindow::createTabList() {
	...
	connect(
		tabListMenu, &QMenu::aboutToShow, this, &MainWindow::fillWindowsMenu
	);
...
void MainWindow::fillWindowsMenu() {
	...
	if (cfg.preferences.mruTabOrder) {
		for (int i = 0; i < mruList.count(); i++) {
			QAction * act = tabListMenu->addAction(
				ui.tabWidget->tabIcon(ui.tabWidget->indexOf(mruList.at(i))),
			...
			if (ui.tabWidget->currentIndex()
				== ui.tabWidget->indexOf(mruList.at(i))) {
				QFont f(act->font());
				f.setBold(true);
				act->setFont(f);
			}
		...
		for (int i = 0; i < ui.tabWidget->count(); i++) {
			QAction * act = tabListMenu->addAction(
				ui.tabWidget->tabIcon(i), ui.tabWidget->tabText(i)
			);
			act->setData(i);
...
ArticleView * MainWindow::createNewTab(bool switchToIt, QString const & name) {
	...
	connect(
		view, &ArticleView::inspectSignal, this, &MainWindow::inspectElement
	);
	...
	connect(
		view,
		&ArticleView::updateFoundInDictsList,
		this,
		&MainWindow::updateFoundInDictsList
	);

	connect(
		view,
		&ArticleView::openLinkInNewTab,
		this,
		&MainWindow::openLinkInNewTab
	);

	connect(
		view,
		&ArticleView::showDefinitionInNewTab,
		this,
		&MainWindow::showDefinitionInNewTab
	);
	...
	connect(
		view,
		&ArticleView::activeArticleChanged,
		this,
		&MainWindow::activeArticleChanged
	);

	connect(
		view,
		&ArticleView::statusBarMessage,
		this,
		&MainWindow::showStatusBarMessage
	);

	connect(
		view, &ArticleView::showDictsPane, this, &MainWindow::showDictsPane
	);

	connect(
		view,
		&ArticleView::forceAddWordToHistory,
		this,
		&MainWindow::forceAddWordToHistory
	);
	...
	connect(
		view,
		&ArticleView::sendWordToInputLine,
		this,
		&MainWindow::sendWordToInputLine
	);

	connect(
		view,
		&ArticleView::storeResourceSavePath,
		this,
		&MainWindow::storeResourceSavePath
	);
	...
	connect(
		view,
		&ArticleView::saveBookmarkSignal,
		this,
		&MainWindow::addBookmarkToFavorite
	);
...
void MainWindow::iconChanged(ArticleView * view, QIcon const & icon) {
	ui.tabWidget->setTabIcon(
		ui.tabWidget->indexOf(view), groupInstances.size() > 1 ? icon : QIcon()
	);
}
...
void MainWindow::showStatusBarMessage(
	QString const & message, int timeout, QPixmap const & icon
) {
	if (message.isEmpty())
...
void MainWindow::tabSwitched(int) {
	...
	if (mruList.size() > 1) {
		int from =
			mruList.indexOf(ui.tabWidget->widget(ui.tabWidget->currentIndex()));
		if (from > 0)
			mruList.move(from, 0);
	}
	...
	if (isWordPresentedInFavorites(
			unescapeTabHeader(headword), cfg.lastMainGroupId
		)) {
		addToFavorites->setIcon(blueStarIcon);
		addToFavorites->setToolTip(tr("Remove current tab from Favorites"));
	} else {
...
void MainWindow::dictionaryBarToggled(bool) {
	...
	); // Visibility change affects searches and results
}
...
void MainWindow::updateFoundInDictsList() {
	...
	if (view) {
		...
		for (QStringList::const_iterator i = ids.constBegin();
			 i != ids.constEnd();
			 ++i) {
			...
			for (unsigned x = dictionaries.size(); x--;) {
				if (dictionaries[x]->getId() == i->toUtf8().data()) {
					...
					QString dictId =
						QString::fromUtf8(dictionaries[x]->getId().c_str());
		...
		if (ui.dictsList->count() > 0
			&& ui.dictsList->selectedItems().empty()) {
...
void MainWindow::editDictionaries(unsigned editDictionaryGroup) {
	...
	{ // Limit existence of newCfg
		...
		EditDictionaries dicts(
			this, newCfg, dictionaries, groupInstances, dictNetMgr
		);

		connect(
			&dicts,
			&EditDictionaries::showDictionaryInfo,
			this,
			&MainWindow::showDictionaryInfo
		);
		...
		dicts.exec();
		cfg.dictionariesDialogGeometry = newCfg.dictionariesDialogGeometry =
			dicts.saveGeometry();
		...
		if (dicts.areDictionariesChanged() || dicts.areGroupsChanged()) {
			...
			for (int x = 0; x < newCfg.groups.size(); x++) {
				...
				if (id != Instances::Group::NoGroupId) {
					...
					if (grp) {
						newCfg.groups[x].mutedDictionaries =
							grp->mutedDictionaries;
						newCfg.groups[x].popupMutedDictionaries =
							grp->popupMutedDictionaries;
					}
				}
			}
...
void MainWindow::editPreferences() {
	...
	if (preferences.exec() == QDialog::Accepted) {
		...
		p.proxyServer.systemProxyUser =
			cfg.preferences.proxyServer.systemProxyUser;
		p.proxyServer.systemProxyPassword =
			cfg.preferences.proxyServer.systemProxyPassword;
		...
		bool needReload =
			(cfg.preferences.displayStyle != p.displayStyle
			 ...
			 || cfg.preferences.alwaysExpandOptionalParts
				 != p.alwaysExpandOptionalParts // DSL format's special feature
			);
...
void MainWindow::updateCurrentGroupProperty() {
	...
	if (grp
		&& translateLine->property("currentGroup").toString() != grp->name) {
...
void MainWindow::translateInputFinished(bool checkModifiers) {
	...
	respondToTranslationRequest(
		Config::InputPhrase(word, translateBoxSuffix), checkModifiers
	);
}

void MainWindow::respondToTranslationRequest(
...
void MainWindow::setTranslateBoxTextAndKeepSuffix(
	QString text, WildcardPolicy wildcardPolicy, TranslateBoxPopup popupAction
) {
...
void MainWindow::setTranslateBoxTextAndClearSuffix(
	QString const & text,
...
bool MainWindow::eventFilter(QObject * obj, QEvent * ev) {
	...
	if (obj == this && ev->type() == QEvent::WindowStateChange) {
		QWindowStateChangeEvent * stev =
			static_cast<QWindowStateChangeEvent *>(ev);
		wasMaximized =
			(stev->oldState() == Qt::WindowMaximized && isMinimized());
	}
	...
	if (ev->type() == QEvent::MouseButtonPress) {
		...
		if (obj != translateBox->wordList()
			&& obj != translateBox->wordList()->viewport()) {
			translateBox->setPopupEnabled(false);
		}
	...
	if (ev->type() == QEvent::KeyPress) {
		...
		bool handleCtrlTab =
			(obj == translateLine || obj == wordList || obj == ui.historyList
			 || obj == ui.favoritesTree || obj == ui.dictsList
			 || obj == groupList);
		...
		if (handleCtrlTab
			...
			)) // Handle only Ctrl+Shist+Tab here because Ctrl+Tab was already handled before
		{
			QApplication::sendEvent(ui.tabWidget, ev);
			return true;
		}
...
void MainWindow::showDefinitionInNewTab(
	QString const & word,
	unsigned group,
	QString const & fromArticle,
	Contexts const & contexts
...
void MainWindow::activeArticleChanged(
	ArticleView const * view, QString const & id
) {
	if (view != getCurrentArticleView())
		return; // It was background action
...
void MainWindow::typingEvent(QString const & t) {
	if (t == "\n" || t == "\r") {
		...
		if (translateLine->isEnabled()) {
			...
			setTranslateBoxTextAndClearSuffix(
				t, WildcardsAreAlreadyEscaped, EnablePopup
			);
...
void MainWindow::showTranslationFor(
	Config::InputPhrase const & phrase,
	unsigned inGroup,
	QString const & scrollTo
) {
...
void MainWindow::latestReleaseReplyReady() {
	...
	if (latestReleaseReply->error() == QNetworkReply::NoError) {
		QString latestReleaseInfo =
			QString::fromUtf8(latestReleaseReply->readAll());
		QRegularExpression firstReleaseAnchor(
			R"(<a\s+[^>]*?class=\"Link--primary\"[^>]*?>[^<]*?<\/a>)",
		...
		if (match.hasMatch()) {
			...
			QRegularExpression extractReleaseRx(
				R"(<a\s+.*?href=\"([^\"]*)\".*?>(.*?)<\/a>)",
				QRegularExpression::DotMatchesEverythingOption
					| QRegularExpression::CaseInsensitiveOption
			);
			...
			if (matchParts.hasMatch()) {
				...
				if (downloadUrl.startsWith("/")) {
					downloadUrl = latestReleaseReply->request().url().host()
						+ downloadUrl;
				}
				success = true;
			}
	...
	if (!success) {
		...
		GD_DPRINTF(
			"Program version's check successful, current version is %ls\n",
			latestVersion.toStdWString().c_str()
		);
	}

	if (success && latestVersion > PROGRAM_VERSION
		&& latestVersion != cfg.skippedRelease) {
		QMessageBox msg(
		...
		QPushButton * dload =
			msg.addButton(tr("Download"), QMessageBox::AcceptRole);
		QPushButton * skip = msg.addButton(
			tr("Skip This Release"), QMessageBox::DestructiveRole
		);
...
void MainWindow::visitForum() {
	QDesktopServices::openUrl(
		QUrl("https://github.com/xiaoyifang/goldendict/discussions")
	);
}
...
void MainWindow::showDictBarNamesTriggered() {
	...
	dictionaryBar.setToolButtonStyle(
		show ? Qt::ToolButtonTextBesideIcon : Qt::ToolButtonIconOnly
	);
...
void MainWindow::setAutostart(bool autostart) {
	...
	const QString destinationPath = QDir::homePath()
		+ "/.config/autostart/goldendict-owned-by-preferences.desktop";
	if (autostart == QFile::exists(destinationPath))
		return; // Nothing to do.
	if (autostart) {
		const QString sourcePath = Config::getProgramDataDir()
			+ "../applications/org.xiaoyifang.GoldenDict_NG.desktop";
...
void MainWindow::on_printPreview_triggered() {
	...
	connect(
		...
		this,
		&MainWindow::printPreviewPaintRequested
	);
...
static void filterAndCollectResources(
	QString & html,
	QRegExp & rx,
	const QString & sep,
	...
	while ((pos = rx.indexIn(html, pos)) != -1) {
		...
		if (resourceIncluded.insert(hash.result()).second) {
			...
			downloadResources.push_back(
				pair<QUrl, QString>(url, folder + host + resourcePath)
			);
		}

		// Modify original url, set to the native one
		resourcePath =
			QString::fromLatin1(QUrl::toPercentEncoding(resourcePath, "/"));
		QString newUrl =
			sep + QDir(folder).dirName() + host + resourcePath + sep;
	...
	view->toHtml([=](QString & html) mutable {
		...
		if (!file.open(QIODevice::WriteOnly)) {
			...
			while ((pos = rx3.indexIn(html, pos)) != -1) {
				QString name =
					QUrl::fromPercentEncoding(rx3.cap(2).simplified().toLatin1()
					);
				QString anchor;
				...
				name = QString("href=\"") + QUrl::toPercentEncoding(name)
					+ ".html" + anchor + "\"";
			...
			if (complete) {
				QString folder = fi.absoluteDir().absolutePath() + "/"
					+ fi.baseName() + "_files";
				...
				filterAndCollectResources(
					html, rx1, "\"", folder, resourceIncluded, downloadResources
				);
				filterAndCollectResources(
					html, rx2, "'", folder, resourceIncluded, downloadResources
				);

				ArticleSaveProgressDialog * progressDialog =
					new ArticleSaveProgressDialog(this);
				// reserve '1' for saving main html file
				int maxVal = 1;

				// Pull and save resources to files
				for (vector<pair<QUrl, QString>>::const_iterator i =
						 downloadResources.begin();
					 i != downloadResources.end();
					 ++i) {
					ResourceToSaveHandler * handler =
						view->saveResource(i->first, i->second);
					if (!handler->isEmpty()) {
						maxVal += 1;
						connect(
							handler,
...
void MainWindow::messageFromAnotherInstanceReceived(QString const & message) {
	...
	if (message.left(15) == "translateWord: ") {
		...
		qWarning() << "Unknown message received from another instance: "
				   << message;
}
...
void MainWindow::phraseReceived(
	Config::InputPhrase const & phrase, WildcardPolicy wildcardPolicy
) {
	toggleMainWindow(true);
	setTranslateBoxTextAndKeepSuffix(
		phrase.phrase, wildcardPolicy, NoPopupChange
	);
...
void MainWindow::headwordReceived(const QString & word, const QString & ID) {
	...
	respondToTranslationRequest(
		Config::InputPhrase::fromPhrase(word),
		false,
		ArticleView::scrollToFromDictionaryId(ID)
	);
}
...
void MainWindow::on_exportHistory_triggered() {
	...
	cfg.historyExportPath = QDir::toNativeSeparators(
...
void MainWindow::on_importHistory_triggered() {
	...
	cfg.historyExportPath =
		QDir::toNativeSeparators(fileInfo.absoluteDir().absolutePath());
	QString errStr;
	QFile file(fileName);
	...
	for (;;) {
		...
		do {
			...
			if ((unsigned)trimmedStr.size() <= history.getMaxItemLength())
				itemList.prepend(trimmedStr);
				 ...
				 && itemList.size() < (int)history.getMaxSize());

		history.enableAdd(true);

		for (QList<QString>::const_iterator i = itemList.constBegin();
			 i != itemList.constEnd();
			 ++i)
		...
		if (fileStream.status() >= QTextStream::ReadCorruptData) {
			...
			mainStatusBar->showMessage(
				errStr, 10000, QPixmap(":/icons/error.svg")
			);
		} else
...
void MainWindow::on_importFavorites_triggered() {
	...
	for (;;) {
		...
		if (fileInfo.suffix().compare("txt", Qt::CaseInsensitive) == 0) {
			if (!ui.favoritesPaneWidget->setDataFromTxt(
					QString::fromUtf8(data.data(), data.size())
				))
				break;
		} else {
			if (!ui.favoritesPaneWidget->setDataFromXml(
					QString::fromUtf8(data.data(), data.size())
				))
				break;
		}
...
void MainWindow::addWordToHistory(const QString & word) {
	if (QRegularExpressionMatch m = RX::Epwing::refWord.match(word);
		m.hasMatch())
		return;
...
void MainWindow::editDictionary(Dictionary::Class * dict) {
	...
	if (!cfg.editDictionaryCommandLine.isEmpty() && !dictFilename.isEmpty()) {
		...
		if (command.contains("%GDWORD%")) {
			QString headword = unescapeTabHeader(
	...
	for (unsigned x = 0; x < dictionaries.size(); x++) {
		if (dictionaries[x]->getId() == id.toUtf8().data()) {
			if (!dictionaries[x]->getDictionaryFilenames().empty()) {
				QDesktopServices::openUrl(
					QUrl::fromLocalFile(dictionaries[x]->getContainingFolder())
				);
			}
			break;
		}
...
void MainWindow::proxyAuthentication(
...
void MainWindow::showFullTextSearchDialog() {
	if (!ftsDlg) {
		ftsDlg = new FTS::FullTextSearchDialog(
			this, cfg, dictionaries, groupInstances, ftsIndexing
		);
		...
		connect(
			ftsDlg,
			SIGNAL(showTranslationFor(QString, QStringList, QRegExp, bool)),
			this,
			SLOT(showTranslationFor(QString, QStringList, QRegExp, bool))
		);
		connect(
			ftsDlg,
		...
		connect(
			&configEvents,
			SIGNAL(mutedDictionariesChanged()),
			ftsDlg,
			SLOT(updateDictionaries())
		);

		unsigned group = groupInstances.empty() ?
			0 :
			groupInstances[groupList->currentIndex()].id;
...
void MainWindow::showFTSIndexingName(QString const & name) {
	if (name.isEmpty())
		...
		mainStatusBar->setBackgroundMessage(
			tr("Now indexing for full-text search: ") + name
		);
}
...
void MainWindow::handleAddToFavoritesButton() {
	...
	if (igrp)
		folder = igrp->favoritesFolder;
	QString headword =
		unescapeTabHeader(ui.tabWidget->tabText(ui.tabWidget->currentIndex()));
...
void MainWindow::addWordToFavorites(
	QString const & word, unsigned groupId, bool exist
) {
	QString folder;
...
void MainWindow::addAllTabsToFavorites() {
	...
	for (int i = 0; i < ui.tabWidget->count(); i++) {
		...
		ui.favoritesPaneWidget->addHeadword(
			folder, unescapeTabHeader(headword)
		);
	}
	addToFavorites->setIcon(blueStarIcon);
	addToFavorites->setToolTip(tr("Remove current tab from Favorites"));
...
bool MainWindow::isWordPresentedInFavorites(
...
void MainWindow::setGroupByName(QString const & name, bool main_window) {
	if (main_window) {
		...
		if (i >= groupList->count())
			gdWarning(
				"Group \"%s\" for main window is not found\n",
				name.toUtf8().data()
			);
	} else {
...
void MainWindow::headwordFromFavorites(
	QString const & headword, QString const & favoritesFolder
) {
	if (!favoritesFolder.isEmpty()) {
		// Find group by it Favorites folder
		for (Instances::Groups::size_type i = 0; i < groupInstances.size();
			 i++) {
	...
	if (words.size() > 1) {
		...
		if (view) {
			view->setDelayedHighlightText(words[1]
			); // findText( words[ 1 ], QWebEnginePage::FindCaseSensitively );
		}
	}
